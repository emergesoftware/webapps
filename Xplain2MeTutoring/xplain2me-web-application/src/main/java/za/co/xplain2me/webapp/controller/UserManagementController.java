...
import za.co.xplain2me.dao.ProfileTypeDAO;
import za.co.xplain2me.dao.ProfileTypeDAOImpl;
...
import za.co.xplain2me.webapp.ObjectFactory;
import za.co.xplain2me.webapp.ObjectFactoryImpl;
...
import za.co.xplain2me.webapp.controller.helper.UserManagementControllerHelper;
...
public class UserManagementController extends GenericController {
    ...
    // search modes
    public static final String SEARCH_MODE_FORM = "search-form";
    public static final String SEARCH_MODE_RESULTS = "search-results";
    
    // search types
    public static final int SEARCH_TYPE_USERNAME = 1;
    public static final int SEARCH_TYPE_ID_NUMBER = 2;
    public static final int SEARCH_TYPE_EMAIL_ADDRESS = 3;
    public static final int SEARCH_TYPE_PROFILE_TYPE = 4;
    public static final int SEARCH_TYPE_ACTIVE_USERS = 5;
    public static final int SEARCH_TYPE_BLOCKED_USERS = 6;
    public static final int SEARCH_TYPE_UNVERIFIED_USERS = 7;
    ...
    /**
     * Delivers the web page to confirm deletion of the user profile
     * 
     * @param request
     * @param profileId
     * @return 
     */
    @RequestMapping(value = RequestMappings.DELETE_USER_PROFILE, 
            method = RequestMethod.GET, 
            params = { "profile-id" })
    public ModelAndView showDeleteUserPage(
        ...
        // return a view
        return createModelAndView(Views.DELETE_USER_PROFILE);
    }
    
    /**
     * Processes the request to delete the user profile.
     * @param request
     * @return 
     */
    @RequestMapping(value = RequestMappings.DELETE_USER_PROFILE, 
            method = RequestMethod.POST)
    public ModelAndView deleteUserProfile(HttpServletRequest request) {
        ...
        // get the password from the request params
        String password = getParameterValue(request, "password");
        // get the value for the confirmation of deletion
        String deleteUserProfile = getParameterValue(request, "deleteUserProfile");
        ...
        if (deleteUserProfile == null || deleteUserProfile.isEmpty() || 
                !deleteUserProfile.equalsIgnoreCase("yes")) {
            errorsEncountered.add("You must tick the confirmation box to confirm"
                    + " this user deletion.");
        }
        ...
        if (errorsEncountered.isEmpty() == false) {
            ...
            // send a redirect 
            return sendRedirect(RequestMappings.DELETE_USER_PROFILE + 
                    "?profile-id=" + form.getProfile().getId()); 
        ...
        // data store
        ProfileDAO profileDao = new ProfileDAOImpl();
        boolean profileDeleted = profileDao.deleteUserProfile(form.getProfile(), 
                context.getProfile());
        ...
        if (profileDeleted) {
            ...
            // log an event
            SystemAuditManager.logAuditAsync(EventTypes.DELETE_USER_PROFILE, 
                    context.getProfile().getPerson().getUser(), 
                    form.getProfile().getId(), 
                    null, 
                    request.getRemoteAddr(), 
            ...
            saveToSessionScope(request, new AlertBlock()
                    .setAlertBlockType(AlertBlock.ALERT_BLOCK_ERROR)
                    .addAlertBlockMessage("The user profile account was deleted "
                            + "successfully."));
            ...
            saveToSessionScope(request, new AlertBlock() 
                        .setAlertBlockType(AlertBlock.ALERT_BLOCK_ERROR)
                        .addAlertBlockMessage("Could not delete the user - please "
                        + "contact your system administrator for gurther assistance."));
    ...
    /**
     * Delivers the webpage to search for users.
     * 
     * @param request
     * @return 
     */
    @RequestMapping(value = RequestMappings.SEARCH_USER, 
            method = RequestMethod.GET)
    public ModelAndView showSearchUsersPage(HttpServletRequest request) {
        
        // check for the mode parameter
        String mode = getParameterValue(request, "mode");
        if (mode == null) mode = SEARCH_MODE_FORM;
        
        if (!mode.equals(SEARCH_MODE_FORM) && 
                !mode.equals(SEARCH_MODE_RESULTS))
            mode = SEARCH_MODE_FORM;
        
        // get the form from the session scope
        UserManagementForm form = (UserManagementForm)
                getFromSessionScope(request, UserManagementForm.class);
        
        // if the form is null but the search mode = SEARCH_MODE_FORM
        if (form == null && mode.equals(SEARCH_MODE_FORM)) 
            form = new UserManagementForm();
        
        // if the form is null but the search mode = SEARCH_MODE_RESULTS
        else if (form == null && mode.equals(SEARCH_MODE_RESULTS)) 
            // send a redirect
            return sendRedirect(RequestMappings.SEARCH_USER + 
                    "?mode=" + SEARCH_MODE_FORM);
        
        // if the form is not null then reset the form values
        // and the search mode = SEARCH_MODE_FORM
        else if (form != null && mode.equals(SEARCH_MODE_FORM)) 
            form.resetForm(); 
        ...
        // load the profile types
        form.setProfileTypes(new TreeMap<Long, ProfileType>());
        ProfileTypeDAO profileTypeDao = new ProfileTypeDAOImpl();
        for (ProfileType type : profileTypeDao.getProfileTypesAuthorisedByProfile(profile))
            form.getProfileTypes().put(type.getId(), type);
        
        // check if there is any alert blocks 
        // in the session scope
        AlertBlock alertBlock = (AlertBlock)
                getFromSessionScope(request, AlertBlock.class);
        if (alertBlock != null && 
        ...
        // return a view
        if (mode.equals(SEARCH_MODE_RESULTS)) 
            return createModelAndView(Views.SEARCH_USERS_RESULTS);
        else 
            return createModelAndView(Views.SEARCH_USERS);
    }
    ...
    @RequestMapping(value = RequestMappings.SEARCH_USER, 
            method = RequestMethod.POST)
    public ModelAndView searchExistingUsers(HttpServletRequest request) {
        
        // get the form from the session scope
        UserManagementForm form = (UserManagementForm)
                getFromSessionScope(request, UserManagementForm.class);
        
        // if the form is null - then
        // send a redirect
        if (form == null) {
            LOG.warning("... the form is null in the session scope ...");
            return sendRedirect(RequestMappings.SEARCH_USER + 
                    "?mode=" + SEARCH_MODE_FORM);
        }
        
        // collect the request parameters
        String searchType = getParameterValue(request, "search-type");
        String searchValue = getParameterValue(request, "search-value");
        
        // validate the search parameters
        List<String> errorsEncountered = helper
                .validateSearchUserParameters(searchType, searchValue);
        
        // if there are errors
        if (errorsEncountered.isEmpty() == false) {
            
            // nullify the user profiles
            form.setUserProfiles(null);
        ...
        // if no errors were found
            ...
            if (Integer.parseInt(searchType) == SEARCH_TYPE_USERNAME)
                searchResults = profileDao.searchUserProfile(
                        SearchUserProfileType.Username, searchValue, profilePerformingAction);
            
            else if (Integer.parseInt(searchType) == SEARCH_TYPE_ID_NUMBER)
                searchResults = profileDao.searchUserProfile(
                        SearchUserProfileType.IdentityOrPassportNumber,
                        searchValue, profilePerformingAction);
            
            else if (Integer.parseInt(searchType) == SEARCH_TYPE_EMAIL_ADDRESS)
                searchResults = profileDao.searchUserProfile(
                        SearchUserProfileType.EmailAddress,
                        searchValue, profilePerformingAction);
            
            else if (Integer.parseInt(searchType) == SEARCH_TYPE_PROFILE_TYPE)
                searchResults = profileDao.searchUserProfile(
                        SearchUserProfileType.ProfileType,
                        Long.parseLong(searchValue), profilePerformingAction);
            
            else if (Integer.parseInt(searchType) == SEARCH_TYPE_ACTIVE_USERS)
                searchResults = profileDao.searchUserProfile(
                        SearchUserProfileType.ActiveUserProfiles, null, 
                        profilePerformingAction);
            
            else if (Integer.parseInt(searchType) == SEARCH_TYPE_BLOCKED_USERS)
                searchResults = profileDao.searchUserProfile(
                        SearchUserProfileType.BlockedUserProfiles, null, 
                        profilePerformingAction);
            
            else if (Integer.parseInt(searchType) == SEARCH_TYPE_UNVERIFIED_USERS)
                searchResults = profileDao.searchUserProfile(
                        SearchUserProfileType.UnverifiedUserProfiles, null, 
                        profilePerformingAction);
            
            else searchResults = new ArrayList<>();
            ...
            if (searchResults == null || searchResults.isEmpty()) {
                ...
                saveToSessionScope(request, new AlertBlock()
                                .setAlertBlockType(AlertBlock.ALERT_BLOCK_ERROR)
                                .addAlertBlockMessage("No user profiles could be found "
                ...
                // transfer the profiles from a list
                // to a tree map
                form.setUserProfiles(new TreeMap<Long, Profile>());
                for (Profile profile : searchResults)
                    form.getUserProfiles().put(profile.getId(), profile);
